...
export function renderForecast(hours, opts) {
    const target = document.getElementById('forecast');
    if (!target)
        return;
    target.innerHTML = '';
    const rated = hours.map((h) => ({ ...h, rating: rateHour(h) }));
...
function updateStatus(hours, windowResult, opts) {
    const overall = document.getElementById('overall-status');
    const best = document.getElementById('best-window');
    const detail = document.getElementById('status-detail');
    if (!overall || !best || !detail)
        return;
...
function renderCalendar(hours, opts) {
    const grid = document.getElementById('calendar-grid');
    if (!grid)
        return;
    const summaries = summarizeByDay(hours, opts.timezone, opts.windUnit);
    const filtered = opts.calendarFilter === 'all'
        ? summaries
        : summaries.filter((s) => s.labels.includes(opts.calendarFilter));
    grid.innerHTML = '';
    if (!filtered.length) {
...
function summarizeByDay(hours, tz, windUnit) {
...
function worstLabel(labels) {
    if (labels.includes('Bad'))
        return 'Bad';
    if (labels.includes('Risky'))
        return 'Risky';
    return 'Good';
}
function toDateKey(dateStr, tz) {
