...
import { HourlyWeather, HourlyRatingResult, RatingLabel } from './types.js';

export interface RatingRuleDescription {
  title: string;
  detail: string;
}

export function rateHour(hour: HourlyWeather): HourlyRatingResult {
  const reasons: string[] = [];
  const warnings: string[] = [];
  let label: RatingLabel = 'Good';
...
export function bestWindow(hours: HourlyWeather[]): { window?: { start: string; end: string; label: RatingLabel }; } {
  const rated = hours.map((h) => ({ hour: h, rating: rateHour(h) }));
  let bestScore = Number.POSITIVE_INFINITY;
  let best: { start: string; end: string; label: RatingLabel } | undefined;
  ...
  for (let i = 0; i < rated.length; i++) {
    for (let len = 1; len <= 3 && i + len <= rated.length; len++) {
      ...
      const avgWind =
        slice.reduce((sum, r) => sum + r.hour.windSpeedMph + r.hour.windGustMph * 0.5, 0) / slice.length;
...
export function ruleDescriptions(): RatingRuleDescription[] {
  return [
...
function maxLabel(a: RatingLabel, b: RatingLabel): RatingLabel {
  const order: Record<RatingLabel, number> = { Good: 0, Risky: 1, Bad: 2 };
